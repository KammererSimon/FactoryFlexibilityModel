...
def simulate_dri(simulation_task, model_parameters, simulation_config, total_sim_count):
    ...
    if simulation.simulated:
        simulation.save(simulation_config["filepath_solved"],
                        ...
                        overwrite=True)
        if simulation_config["enable_log_simulation_iteration"]:
            print(f"{round(simulation_task['simulation_number']/total_sim_count*100, 2)}%     Simulation number {simulation_task['simulation_number']} finished within: {round((time.time()-t_start), 2)} s")
    else:
        simulation.save(simulation_config["filepath_problem"],
                        name=simulation_filename,
                        overwrite=True)
        if simulation_config["enable_log_simulation_iteration"]:
            print(f"{round(simulation_task['simulation_number']/total_sim_count*100, 2)}%     Simulation number {simulation_task['simulation_number']} aborted due to solver timeout")
