...
def show_icon_selection_dialog(app, caller):
    ...
    if app.selected_asset["type"] == "source":
        icon_list = app.source_icons
        size = [100, 100]
    elif app.selected_asset["type"] == "sink":
        icon_list = app.sink_icons
        size = [100, 10]
    else:
        icon_list = app.component_icons
        size = [248, 100]

    for icon_name, icon_source in icon_list.items():
        image = Image(source=icon_source,
                      size_hint=(None,None),
                      size=size,
                      on_touch_up=lambda instance, touch, icon_source=icon_source:  set_image(instance, touch, caller, icon_source, app))
