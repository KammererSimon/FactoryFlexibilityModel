...
from kivymd.uix.list import IconLeftWidget, IconRightWidget, TwoLineAvatarIconListItem, OneLineIconListItem, IconLeftWidgetWithoutTouch
from factory_flexibility_model.ui.gui_components.layout_flowtype_configuration.dialog_flowtype_definition.dialog_flowtype_definition import show_flowtype_config_dialog
...
def save_changes_on_flowtype(app):
    ...
    if (
        not app.dialog.content_cls.ids.textfield_flowtype_name.text
        == flowtype.name
    ):
    ...
    flowtype.color = color(
        app.dialog.content_cls.ids.icon_flowtype_color.icon_color
    )
...
def update_flowtype_list(app):
    ...
    for flowtype in app.blueprint.flowtypes.values():
        ...
        list_item.bind(on_release=lambda x, item=list_item: select_flowtype_list_item(app, item))
