...
def initialize_visualization(app, *args):
    ...
    for component in app.blueprint.components.values():
        ...
        if component["type"] == "pool":
            ...
            component_framelabel.source = "factory_flexibility_model\\ui\\assets\\empty_rectangle.png"
            ...
            inner_circle = Ellipse(
                ...
                size=(
                    component_height
                    - 32 * app.session_data["display_scaling_factor"],
                    component_height
                    - 32 * app.session_data["display_scaling_factor"],
                ),
            )
    ...
    for component in app.blueprint.components.values():
        app.root.ids[f"frame_{component['key']}"].bind(on_touch_move=lambda x, y: update_visualization(app))
        app.root.ids[f"frame_{component['key']}"].bind(on_touch_up=lambda touch, instance: app.click_on_component(touch, instance))
...
def update_visualization(app, *args, **kwargs):
    ...
    if (
        ...
        app.root.ids["highlight"].source = app.highlight_icons[
    ...
    if app.connection_edit_mode:
        if "mouse_pos" in kwargs:
            ...
            app.root.ids[f"line_preview"].points = (
                app.root.ids[f"frame_{app.selected_asset['key']}"].pos[0] + component_width[app.selected_asset["type"]] / 2,
                app.root.ids[f"frame_{app.selected_asset['key']}"].pos[1] + component_height / 2,
                end_x,
                end_y)
